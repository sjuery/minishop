...
$fd_basquet = fopen("./data/basquet.tsv", "r");
if (!$fd_account || !$fd_admin || !$fd_item || !$fd_basquet)
{
	$_SESSION['error'] = "failed to open data file";
...
$basquet_key = explode("\t", fgets($fd_basquet));
for ($n = 0; ($line = fgets($fd_basquet)) !== false; $n++) 
{
	$tmp = explode("\t", $line);
...
<body>
	<header>
		<strong class="title"><?php if ($_SESSION['login']) print $_SESSION['login']." ! "; ?>Welcome To The Fruit Black Market</strong>
		<?php
	...
	<form action="basquet_add.php" method="post">
	<?php
	foreach ($_SESSION['item'] as $item)
	{
		...
		if ($item['name'] === "Blue Apple" && ($_SESSION['category'] === "all" || $_SESSION['category'] === "blue"))
		{
			echo "
			<table>
				<td>
		...
		if ($item['name'] === "Blue Banana" && ($_SESSION['category'] === "all" || $_SESSION['category'] === "blue"))
		{
			echo "
			<table>
				<td>
		...
		if ($item['name'] === "Yellow Banana" && ($_SESSION['category'] === "all" || $_SESSION['category'] === "yellow"))
		{
			echo "
			<table>
				<td>
